...
def GetYear():
    ...
    global CsvName 
    CsvName = "PrayerTiming_v2_"+year+".csv"
    ...
    for city in cities:
        ...
        for y in range(1):
            ...
            while True:
                ...
                try:
                    url='https://www.timeanddate.com/sun/italy/'+city+'?month='+month+'&year='+year
                    ...
                    print("ERROR:Value error, please check spellings,",year, month,"or", city, "is not found... Exiting code!!!")
                    input('\nPress key to exit.')
                    # System exit
                    #sys.exit()
                    continue
            ...
            for i in range(NumDays):
                ...
                NamazDelta = []
                if(PrayerArray[i,0])
                SunriseData = PrayerArray[i,1].split()
                print(SunriseData)
                SunriseData[0] = SunriseData[0].replace('.', ':')
                # Sunrise time data 
                SunriseTime = datetime.strptime(SunriseData[0], '%H:%M').time()
                ...
                DayLightData = PrayerArray[i,3]
                print(DayLightData)
                DayLightData = DayLightData.replace('.', ':')
                # Total daylight hours
                DayTime = datetime.strptime(DayLightData, '%H:%M:%S').time()
                ...
                SunSetData = PrayerArray[i,2].split()
                SunSetData[0] = SunSetData[0].replace('.', ':')
                # Sunset time data
                SunsetTime = datetime.strptime(SunSetData[0], '%H:%M').time()
                ...
                HeadDataframe.loc[i, :] = [PrayerArray[i,0], NamazStr[1], NamazStr[0], NamazStr[2], NamazStr[3], NamazStr[4], NamazStr[5], city]
            # Print dictionary
            # print(HeadDataframe)
            print("Data copy of",city,"for month", month)
            try:
